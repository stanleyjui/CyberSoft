...
namespace Cybersoft.Biz.Cust
{
    ...
    public class PBBGMC001
    {
        ...
        int i = 0;
        int GMGAS_TXT_Count = 0;

        //竹名瓦斯約定檔欄位
        String[] GMGAS_CHGOUT = null; 
        ...
        public string RUN()
        {
            ...
            try
            {
                ...
                //寫出檔案路徑
                FILE_PATH = CMCURL.getPATH("GMGAS_CHGOUT");

                if (FILE_PATH == "")
                {
                    logger.strJobQueue = "路徑取得錯誤!!! GMGAS_CHGOUT路徑為 " + FILE_PATH;
                ...
                else
                {
                    ...
                    logger.strJobQueue = " GMGAS_CHGOUT路徑為 " + FILE_PATH;
                    logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                }
                #endregion

                #region 擷取今日需送約定檔之竹名瓦斯資料
                PUBLIC_APPLY.init();
                PUBLIC_APPLY.strWherePAY_TYPE = "0035";
                ...
                switch (PUBLIC_APPLY_RC)
                {
                    case "S0000": //查詢成功
                        ...
                        GMGAS_CHGOUT = new string[PUBLIC_APPLY_Query_Count];
                        break;

                    case "F0023": //無需處理資料
                        logger.strJobQueue = "今日無竹名瓦斯約定檔需產生;";
                        logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                        break;
                ...
                for (i = 0; i < PUBLIC_APPLY.resultTable.Rows.Count; i++)
                {
                    ...
                    //組明細
                    GMGAS_CHGOUT[i] = CHANGE_TYPE + PAY_ACCT_NBR + PAY_NBR + new string(' ', 1) + CHANGE_DTE;
                    //將變數值還原
                    CHANGE_TYPE = "0";
                    ...
                    PUBLIC_APPLY.strMNT_USER = "PBBGMC001";
                ...
                #region 產生約定檔 陣列 --> 220Z_yyyymmdd_5949_001
                ...
                using (StreamWriter srOutFile = new StreamWriter(fsOutFile, BIG5))
                {
                    ...
                    {
                        srOutFile.Write(GMGAS_CHGOUT[k]);
                        srOutFile.Write("\r\n");
                        srOutFile.Flush();
                        GMGAS_TXT_Count = GMGAS_TXT_Count + 1;
                    }
                    srOutFile.Close();
                }
                fsOutFile.Close();
        ...
        void writeDisplay()
        {
            ...
            //竹名瓦斯約定檔筆數(含頭尾)
            logger.strTBL_NAME = "GMGAS_CHGOUT";
            logger.strTBL_ACCESS = "I";
            logger.intTBL_COUNT = GMGAS_TXT_Count;
            logger.writeCounter();
