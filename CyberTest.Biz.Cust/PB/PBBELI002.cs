...
namespace Cybersoft.Biz.Cust
{
    ...
    public class PBBELI002
    {
        ...
        //序號組合
        String TRANS_DTE = "";
        String PAY_TYPE = "0004";   //台電(高壓電)
        String WK_BATCH = "5";      //固定為5,若當天要處理兩批,第二批請改6
            ...
            try
            {
                ...
                if (Check_RC.Substring(0, 5) != "S0000")
                {
                    if (ELECT_PAY_DTE.Contains(TODAY_PROCESS_DTE.ToString("yyyyMMdd")))
                    {
                        #region 發提示Email
                        //Email主旨
                        string email_subject = "::: CyberCArd Message for PUBLIC_IN/PBBELI002  FILE NOTICE  :::  (Notice ONLY!)";
                        //Email內容
                        #region Email內容
                ...
                using (StreamReader srInFile = new StreamReader(strOutFileName, BIG5))
                {
                    ...
                    while ((strInfData = srInFile.ReadLine()) != null && PROCESS_FLAG == "Y")
                    {
                        ...
                        {
                            logger.strJobQueue = "台電(高壓電)檔中第" + intReadInfDataCount + "筆的長度有誤，請通知系統人員! 該筆實際長度為 " + strInfData.Length;
                            logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                            return "B0099:" + logger.strJobQueue;
                        }
                        ...
                        switch (strTag)
                        {
                            case "1":  //檔頭資料
                                ...
                                if (PROCESS_FLAG == "Y")
                                {
                                    ...
                                    #region 刪除今日新增之台電(高壓電)代繳清單(rerun準備)
                                    PUBLIC_LIST.init();
                            ...
                            case "2":  //明細資料
                                ...
                                PUBLIC_HIST_T.resultTable.Rows[x]["MNT_USER"] = "PBBELI002";
