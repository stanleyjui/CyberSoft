...
{
    /// <summary>
    /// 將停車費退費(台北市、台北縣、台南市、高雄市)資料寫入代繳紀錄檔中(營業日執行)
    /// ABEND 處理:修正問題後可重新執行,若一日有兩批需處理,需改批號;但若該批資料已至授權階段將不可再執行
    /// </summary>
    public class PBBPKI002
    {
        ...
        #region 宣告常數
        const string strJOBNAME = "PBBPKI002";
        #endregion
        ...
        const int intDataLength = 130;  //檔案Detail長度
        ...
        public string RUN(String strSOURCE, string BATCH_NO)
        {
            ...
            try
            {
                ...
                switch (strSOURCE.ToString().Trim())
                {
                    case "TPE":     //台北市停車費
                        //收取檔案路徑
                        FILE_PATH = CMCURL.getPATH("TPE_RTNIN");
                        PAY_TYPE = "0023";
                        break;

                    case "TPC":     //台北縣停車費
                        //收取檔案路徑
                        FILE_PATH = CMCURL.getPATH("TPC_RTNIN");
                        PAY_TYPE = "0021";
                        break;

                    case "PRK":     //台南市停車費
                        //收取檔案路徑
                        FILE_PATH = CMCURL.getPATH("PRK_RTNIN");
                        PAY_TYPE = "0006";
                        break;

                    case "KAO":     //高雄市停車費
                        //收取檔案路徑
                        FILE_PATH = CMCURL.getPATH("KAO_RTNIN");
                        PAY_TYPE = "0022";
                        break;
        ...
        {
            #region 宣告檔案路徑
            #region 刪除今日新增之停車費代繳資料(for rerun PUBLIC_HIST)
            ...
            if (xml.strMSG.Length > 0)
            {
                logger.strJobQueue = "[Xml2DataTable(REC_R)] - " + xml.strMSG.ToString().Trim();
                logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                throw new System.Exception("B0099：" + logger.strJobQueue);
            }
            ...
            if (xml.strMSG.Length > 0)
            {
                logger.strJobQueue = "[Xml2DataTable(REC_RT)] - " + xml.strMSG.ToString().Trim();
                logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                throw new System.Exception("B0099：" + logger.strJobQueue);
            }
                ...
                while ((strInfData = srInFile.ReadLine()) != null && PROCESS_FLAG == "Y")
                {
                    ...
                    switch (strTag)
                    {
                        case "1":  //檔頭資料
                            ...
                            #region 檔頭檢核 (暫不檢核)
                            PAY_DTE = NEXT_PROCESS_DTE;
                            PROCESS_FLAG = "Y";

                            ////處理日期必須大於今日批次日,若等於今日批次日需人工介入
                            //if (InfData_DataTable.Rows[0]["PAY_DTE"].ToString() == "0")
                            //{
                            //    PAY_DTE = NEXT_PROCESS_DTE;
                            //    PROCESS_FLAG = "Y";
                            //    logger.strJobQueue = "入扣帳日為" + PAY_DTE + "等於下個批次日" + NEXT_PROCESS_DTE + "!!!";
                            //    logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                            //}
                            //else
                            //{
                            ...
                            if (PROCESS_FLAG == "Y")
                            {
                                ...
                                #region 刪除今日新增之停車費退費清單(rerun準備)
                                PUBLIC_LIST.init();
                                PUBLIC_LIST.strWherePROCESS_DTE = NEXT_PROCESS_DTE.ToString("yyyyMMdd");
                                PUBLIC_LIST.strWhereRETURN_DTE = NEXT_PROCESS_DTE.ToString("yyyyMMdd");
                        ...
                        case "2":  //明細資料
                            ...
                            #region 檢核明細資料
                            //PAY_NBR = Convert.ToString(InfData_DataTable.Rows[0]["PAY_NBR"]).Substring(0, 7);
                            PAY_NBR = "";
                            ...
                            //組扣款序號 檔案傳送日期(6碼,年為西元後兩碼)+代繳類別(4碼)+批號(1碼)+退貨註記(R)+序號(4碼) = 16碼
                            ...
                            PUBLIC_HIST_T.resultTable.Rows[x]["USER_FIELD_1"] = "R";
