...
{
    /// <summary>
    /// 產生省水扣帳回應檔,並將入帳交易先寫至TX_WAREHOUSE,待下一營業日入帳
    /// </summary>
    public class PBBTWO001
    {
        ...
        int intRECCount = 0;
        int intREC_TCount = 0;
        int TWWATER_TOTAL_CNT = 0;
        Decimal TWWATER_TOTAL_AMT = 0;
        int TX_WAREHOUSE_T_Count_Insert = 0;
        int TWWATER_TXT_Count = 0;
        ...
        //省水回應檔欄位
        String[] TWWATER_STMTOUT = null;
        String PAY_RESULT_TWWATER = "";
        Decimal TWWATER_SUCCESS_AMT = 0;
        Decimal TWWATER_FAIL_AMT = 0;
        int TWWATER_SUCCESS_CNT = 0;
        int TWWATER_FAIL_CNT = 0;
        ...
        public string RUN()
        {
            ...
            try
            {
                ...
                //XML名稱
                strXML_Layout = CMCURL.getURL("PUBLIC_TWWATER.xml");
                //寫出檔案路徑
                FILE_PATH = CMCURL.getPATH("TWWATER_STMTOUT");

                if (strXML_Layout == "" || FILE_PATH == "")
                {
                    logger.strJobQueue = "路徑取得錯誤!!! PUBLIC_TWWATER.xml路徑為 " + strXML_Layout + " TWWATER_STMTOUT路徑為 " + FILE_PATH;
                ...
                else
                {
                    ...
                    logger.strJobQueue = "PUBLIC_TWWATER.xml路徑為 " + strXML_Layout + " TWWATER_STMTOUT路徑為 " + FILE_PATH;
                    logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                }
                #endregion
                ...
                #region 擷取今日需送回應檔之省水資料
                PUBLIC_LIST.init();
                PUBLIC_LIST.strWherePAY_TYPE = "0003";
                ...
                switch (PUBLIC_LIST_RC)
                {
                    case "S0000": //查詢成功
                        ...
                        TWWATER_STMTOUT = new string[PUBLIC_LIST_Query_Count];
                        break;

                    case "F0023": //無需處理資料
                        logger.strJobQueue = "今日無省水回應檔需產生;今日需送回應檔時間為大於 " + PREV_PROCESS_DTE + "且小於等於" + TODAY_PROCESS_DTE;
                        logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                        return "B0000";
                ...
                for (i = 0; i < PUBLIC_LIST.resultTable.Rows.Count; i++)
                {
                    ...
                    switch (strTag)
                    {
                        case "1":  //檔頭資料       
                            ...
                            #region  組檔頭
                            TWWATER_STMTOUT[i] = PAY_DATA_AREA.Substring(0, 120);
                            #endregion

                            #region 刪除TX_WAREHOUSE,RERUN 機制
                            TX_WAREHOUSE.init();
                            TX_WAREHOUSE.strWhereSOURCE_CODE = "PU00000003";
                            TX_WAREHOUSE.DateTimeWhereMNT_DT = TODAY_PROCESS_DTE;
                            TX_WAREHOUSE.strWhereMNT_USER = "PBBTWO001";
                        ...
                        case "2":  //明細資料
                            ...
                            if (PAY_RESULT == "S000")
                            {
                                PAY_RESULT_TWWATER = "00";  //扣帳成功
                                TWWATER_SUCCESS_CNT = TWWATER_SUCCESS_CNT + 1;
                                TWWATER_SUCCESS_AMT = TWWATER_SUCCESS_AMT + TX_AMT;
                                TWWATER_STMTOUT[i] = PAY_DATA_AREA.Substring(0, 62) + PAY_RESULT_TWWATER + PAY_DATA_AREA.Substring(64, 56);
                                ...
                                if (TX_AMT > 0)
                                {
                                    ...
                                    TX_WAREHOUSE_T.resultTable.Rows[x]["SOURCE_CODE"] = "PU00000003";
                                    ...
                                    //偷偷把水號遮住
                                    //TWWATER_MASK_STRING = CMCNBR001.GET_MASK(PAY_NBR, MASK_TYPE);
                                    TX_WAREHOUSE_T.resultTable.Rows[x]["DESCR"] = Strings.StrConv("台灣省自來水公司" + PAY_NBR, VbStrConv.Wide);
                                    ...
                                    TX_WAREHOUSE_T.resultTable.Rows[x]["MNT_USER"] = "PBBTWO001";
                                }
                                #endregion
                                ...
                                PUBLIC_HIST.strMNT_USER = "PBBTWO001";
                            ...
                            if (PAY_RESULT == "I001")
                            {
                                PAY_RESULT_TWWATER = "02";   //未申請代扣
                                TWWATER_FAIL_CNT = TWWATER_FAIL_CNT + 1;
                                TWWATER_FAIL_AMT = TWWATER_FAIL_AMT + TX_AMT;
                                TWWATER_STMTOUT[i] = PAY_DATA_AREA.Substring(0, 62) + PAY_RESULT_TWWATER + PAY_DATA_AREA.Substring(64, 56);
                                //寫出成功/失敗明細報表
                                insertReport_TABLE();
                                break;
                            ...
                            if (PAY_RESULT == "I008")
                            {
                                PAY_RESULT_TWWATER = CTL_CODE;  //公用事業單位止扣
                                TWWATER_FAIL_CNT = TWWATER_FAIL_CNT + 1;
                                TWWATER_FAIL_AMT = TWWATER_FAIL_AMT + TX_AMT;
                                TWWATER_STMTOUT[i] = PAY_DATA_AREA.Substring(0, 62) + PAY_RESULT_TWWATER + PAY_DATA_AREA.Substring(64, 56);
                                //寫出成功/失敗明細報表
                        ...
                        case "3": //檔尾資料
                            ...
                            TWWATER_TOTAL_AMT = TWWATER_FAIL_AMT + TWWATER_SUCCESS_AMT;
                            TWWATER_TOTAL_CNT = TWWATER_FAIL_CNT + TWWATER_SUCCESS_CNT;

                            //檢查金額
                            if (TOTAL_AMT != TWWATER_TOTAL_AMT)
                            {
                                logger.strJobQueue = "回應檔總金額和原始檔不合,請確認!!!";
                                logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                                logger.strJobQueue = "原始檔總金額 = " + TOTAL_AMT + "回應檔總金額 = " + TWWATER_TOTAL_AMT;
                                logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                                return "B0099:" + logger.strJobQueue;
                            }

                            //檢查筆數
                            if (TOTAL_CNT != TWWATER_TOTAL_CNT)
                            {
                                logger.strJobQueue = "回應檔總筆數和原始檔不合,請確認!!!";
                                logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                                logger.strJobQueue = "原始檔總筆數 = " + TOTAL_CNT + "回應檔總筆數 = " + TWWATER_TOTAL_CNT;
                                logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                                return "B0099:" + logger.strJobQueue;
                            }
                            ...
                            #region 組檔尾
                            TEMP_SUCCESS_AMT = Convert.ToInt32(TWWATER_SUCCESS_AMT * 100).ToString().PadLeft(16, '0');
                            TEMP_FAIL_AMT = Convert.ToInt32(TWWATER_FAIL_AMT * 100).ToString().PadLeft(16, '0');

                            TWWATER_STMTOUT[i] = PAY_DATA_AREA.Substring(0, 26) + TEMP_SUCCESS_AMT + TWWATER_SUCCESS_CNT.ToString().PadLeft(10, '0')
                                             + TEMP_FAIL_AMT + TWWATER_FAIL_CNT.ToString().PadLeft(10, '0') + PAY_DATA_AREA.Substring(78, 42);
                            #endregion

                            break;

                        default: //資料庫錯誤
                            return "B0016:" + "省水代扣繳 REC_TYPE ERROR " + strTag;
                ...
                using (StreamWriter srOutFile = new StreamWriter(fsOutFile, BIG5))
                {
                    ...
                    {
                        srOutFile.Write(TWWATER_STMTOUT[k]);
                        srOutFile.Write("\r\n");
                        srOutFile.Flush();
                        TWWATER_TXT_Count = TWWATER_TXT_Count + 1;
                    }
                    srOutFile.Close();
                }
                fsOutFile.Close();
        ...
        void insertReport_TABLE()
        {
            ...
            REPORT_TABLE.Rows[k]["PAY_YYMM"] = BILL_DTE.PadLeft(5, '1').Substring(0, 3) + "-" + BILL_DTE.PadLeft(5, '1').Substring(3, 2);
        ...
        void writeReport()
        {
            ...
            Decimal TWWATER_SUCCESS_FEE = 0;
            Decimal TWWATER_SUCCESS_NET_AMT = 0;
            TWWATER_SUCCESS_FEE = Convert.ToDecimal(TWWATER_SUCCESS_CNT * 3);
            TWWATER_SUCCESS_NET_AMT = TWWATER_SUCCESS_AMT - TWWATER_SUCCESS_FEE;

            //設定特殊欄位資料
            ArrayList alSumData = new ArrayList();
            alSumData.Add(new ExcelFactory.ListItem("#RPT_DATE", TODAY_PROCESS_DTE.ToString("yyyy/MM/dd")));
            alSumData.Add(new ExcelFactory.ListItem("#TOTAL_CNT", TWWATER_TOTAL_CNT + " 筆"));
            alSumData.Add(new ExcelFactory.ListItem("#TOTAL_AMT", TWWATER_TOTAL_AMT));
            alSumData.Add(new ExcelFactory.ListItem("#SUCCESS_CNT", TWWATER_SUCCESS_CNT + " 筆"));
            alSumData.Add(new ExcelFactory.ListItem("#SUCCESS_AMT", TWWATER_SUCCESS_AMT));
            alSumData.Add(new ExcelFactory.ListItem("#FAIL_CNT", TWWATER_FAIL_CNT + " 筆"));
            alSumData.Add(new ExcelFactory.ListItem("#FAIL_AMT", TWWATER_FAIL_AMT));
            alSumData.Add(new ExcelFactory.ListItem("#SUCCESS_FEE", TWWATER_SUCCESS_FEE.ToString("0")));
            alSumData.Add(new ExcelFactory.ListItem("#NET_SUCC_AMT", TWWATER_SUCCESS_NET_AMT.ToString("0")));
            //設定合併欄位資料
            ArrayList alMegData = new ArrayList();

            //產出報表
            CMCRPT001.Output(REPORT_TABLE, alSumData, alMegData, "PBRTWO001(台灣自來水扣繳成功失敗報表)", "PBRTWO001", "Sheet1", "Sheet1", TODAY_PROCESS_DTE, true);
        }
        #endregion
        ...
        void writeDisplay()
        {
            ...
            //省水回應檔筆數(含頭尾)
            logger.strTBL_NAME = "TWWATER_STMTOUT";
            logger.strTBL_ACCESS = "I";
            logger.intTBL_COUNT = TWWATER_TXT_Count;
            logger.writeCounter();
