...
namespace Cybersoft.Biz.Cust
{
    ...
    public class PBBPWI001
    {
        ...
        const int intHeaderLength = 43;  //檔案長度
        const int intDataLength = 98;  //檔案長度
        ...
        public string RUN(string BATCH_NO)
        {
            ...
            try
            {
                ...
                //收取檔案路徑
                FILE_PATH_D = CMCURL.getPATH("TPWATER_STMTIN_D");

                if (strXML_Layout == "" || FILE_PATH_D == "")
                {
                    logger.strJobQueue = "路徑取得錯誤!!! PUBLIC_TPWATER.xml路徑為 " + strXML_Layout + " ,TPWATER_STMTIN_D路徑為 " + FILE_PATH_D;
                ...
                else
                {
                    ...
                    logger.strJobQueue = "PUBLIC_TPWATER.xml路徑為 " + strXML_Layout + " ,TPWATER_STMTIN_D路徑為 " + FILE_PATH_D;
                    logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                }

                //收取檔案路徑
                FILE_PATH_H = CMCURL.getPATH("TPWATER_STMTIN_H");

                if (strXML_Layout == "" || FILE_PATH_H == "")
                {
                    logger.strJobQueue = "路徑取得錯誤!!! PUBLIC_TPWATER.xml路徑為 " + strXML_Layout + " ,TPWATER_STMTIN_H路徑為 " + FILE_PATH_H;
                    logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                    return "B0099:" + logger.strJobQueue;
                }
                else
                {
                    ...
                    logger.strJobQueue = "PUBLIC_TPWATER.xml路徑為 " + strXML_Layout + " ,TPWATER_STMTIN_H路徑為 " + FILE_PATH_H;
                    logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                }

                string Check_H_RC = CMCURL.isFileExists(FILE_PATH_H).ToString();
                string Check_D_RC = CMCURL.isFileExists(FILE_PATH_D).ToString();
                if (!(Check_H_RC.Substring(0, 5) == "S0000" && Check_D_RC.Substring(0, 5) == "S0000"))
                {
                    if (TODAY_PROCESS_DTE.DayOfWeek.ToString("d") == "3")
                    {
                        #region 發提示Email
                        //Email主旨
                        string email_subject = "::: CyberCArd Message for PUBLIC_IN/PBBPWI001  FILE NOTICE  :::  (Notice ONLY!)";
                        //Email內容
                        #region Email內容
                        string email_body = "此Mail僅為通知，批次仍在執行中！           " + "</br>" + "</br>" + "</br>" +
                                            "TIME     = " + DateTime.Now.ToString() + "</br>" +
                                            "本週無市水扣款檔案，請注意前一工作日是否已處理，謝謝。";
                        #endregion
                        Cybersoft.Base.CMCJOB001 CMCJOB001 = new Cybersoft.Base.CMCJOB001();
                        //取得主機名稱
                        CMCJOB001.strJobEnv = System.Net.Dns.GetHostName();
                        string resultMessage = CMCJOB001.processMAIL(email_subject,  email_body, false);
                        if (resultMessage != "S0000")
                        {
                    ...
                    logger.strJobQueue = Check_H_RC.Trim() + "; " + Check_D_RC.Trim();
                    logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                    return "B0000";
                }
                #endregion

                #region 刪除今日新增之北市水代繳資料
                String WK_SEQ_11 = TODAY_PROCESS_DTE.ToString("yyMMdd") + PAY_TYPE + WK_BATCH;
                PUBLIC_HIST.strWhereMNT_USER = "PBBPWI001";
                ...
                using (StreamReader srInFile = new StreamReader(strOutFileName, BIG5))
                {
                    ...
                    while ((strInfData = srInFile.ReadLine()) != null && PROCESS_FLAG == "Y")
                    {
                        ...
                        if (strInfData.Length != intHeaderLength)
                        {
                            logger.strJobQueue = "北市水檔中第" + intReadInfDataCount + "筆的長度有誤，請通知系統人員! 該筆實際長度為 " + strInfData.Length;
                            logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                            return "B0099:" + logger.strJobQueue;
                        }
                        ...
                        //WK_PAY_DTE = Convert.ToString(InfData_DataTable.Rows[0]["PAY_DTE"]);
                        ...
                        //{
                        //    PROCESS_FLAG = "Y";
                        //    logger.strJobQueue = "指定入扣帳日為" + PAY_DTE + "大於今日批次日" + TODAY_PROCESS_DTE + "符合處理日期區間!!!";
                        //    logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                        //}
                        if (PAY_DTE == TODAY_PROCESS_DTE)
                        {
                            PROCESS_FLAG = "Y";
                            logger.strJobQueue = "指定入扣帳日為" + PAY_DTE + "等於今日批次" + TODAY_PROCESS_DTE + ", 符合處理日期區間!!!";
                            logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                        }
                        else
                        {
                            PROCESS_FLAG = "N";
                            logger.strJobQueue = "指定入扣帳日為" + PAY_DTE + "不等於今日批次日" + TODAY_PROCESS_DTE + ", 不符合處理日期區間!!!";
                            logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                        }
                        //if (PAY_DTE < TODAY_PROCESS_DTE)
                        //{
                        //    PROCESS_FLAG = "N";
                        //    logger.strJobQueue = "指定入扣帳日為" + PAY_DTE + "小於今日批次日" + TODAY_PROCESS_DTE + "不符合處理日期區間!!!";
                        //    logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                        //}                       
                        #endregion
                        ...
                        if (PROCESS_FLAG == "Y")
                        {
                            ...
                            if (PROCESS_FLAG == "Y")
                            {
                                ...
                                PUBLIC_LIST_T.resultTable.Rows[i]["PAY_DATA_AREA"] = strInfData.Substring(0, intHeaderLength).ToString();
                                PUBLIC_LIST_T.resultTable.Rows[i]["MNT_DT"] = TODAY_PROCESS_DTE;
                                PUBLIC_LIST_T.resultTable.Rows[i]["MNT_USER"] = "PBBPWI001";
                            }
                            #endregion
                ...
                if (PROCESS_FLAG == "Y")
                {
                    using (StreamReader srInFile = new StreamReader(strOutFileName, BIG5))
                    {
                        ...
                        while ((strInfData = srInFile.ReadLine()) != null && PROCESS_FLAG == "Y")
                        {
                            ...
                            {
                                logger.strJobQueue = "北市水檔中第" + intReadInfDataCount + "筆的長度不為" + intDataLength + "，請通知系統人員!該筆實際長度為 " + strInfData.Length;
                                logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                                return "B0099:" + logger.strJobQueue;
                            }
                            ...
                            PAY_ACCT_NBR = Convert.ToString(InfData_DataTable.Rows[0]["PAY_ACCT_NBR"]).Substring(3, 14);
                            //取得信用卡卡號
                            TRANSFER_PAY_NBR();
                            ...
                            PUBLIC_HIST_T.resultTable.Rows[x]["PAY_RESULT"] = "";
                            PUBLIC_HIST_T.resultTable.Rows[x]["MNT_DT"] = TODAY_PROCESS_DTE;
                            PUBLIC_HIST_T.resultTable.Rows[x]["MNT_USER"] = "PBBPWI001";
                            ...
                            if (PROCESS_FLAG == "Y")
                            {
                                ...
                                //組扣款序號 檔案傳送日期(6碼,年為西元後兩碼)+代繳類別(4碼)+批號(1碼)+序號(6碼) = 16碼
                                WK_SEQ = new StringBuilder();
                                WK_SEQ.Append(TRANS_DTE);
                                WK_SEQ.Append(PAY_TYPE);
                                WK_SEQ.Append(WK_BATCH);
                ...
                if (PUBLIC_HIST_T.resultTable.Rows.Count > 0 && PROCESS_FLAG == "Y")
                {
                    //先紀錄筆數
                ...
                if (PUBLIC_LIST_T.resultTable.Rows.Count > 0 && PROCESS_FLAG == "Y")
                {
                    //先紀錄筆數
        ...
        void writeDisplay()
        {
            ...
            //今日處理之北市水資料
            logger.strTBL_NAME = "PUBLIC_TPWATER";
