...
namespace Cybersoft.Biz.Cust
{
    ...
    public class PBBELI001
    {
        ...
        string ELECT_PAY_DTE = "20120104,20120109,20120113,20120119," +
                               "20120204,20120210,20120216,20120222," +
                               "20120306,20120312,20120316,20120322," +
                               "20120406,20120412,20120418,20120424," +
                               "20120507,20120511,20120517,20120523," +
                               "20120606,20120612,20120618,20120622," +
                               "20120705,20120711,20120717,20120723," +
                               "20120806,20120810,20120816,20120822," +
                               "20120906,20120912,20120918,20120924," +
                               "20121004,20121011,20121017,20121023," +
                               "20121106,20121112,20121116,20121122," +
                               "20121206,20121212,20121218,20121224";
        ...
        public string RUN(string BATCH_NO)
        {
            ...
            try
            {
                ...
                //收取檔案路徑
                FILE_PATH = CMCURL.getPATH("ELECT_STMTIN");

                if (strXML_Layout == "" || FILE_PATH == "")
                {
                    logger.strJobQueue = "路徑取得錯誤!!! PUBLIC_ELECT.xml路徑為 " + strXML_Layout + " ELECT_STMTIN路徑為 " + FILE_PATH;
                ...
                else
                {
                    ...
                    logger.strJobQueue = "PUBLIC_ELECT.xml路徑為 " + strXML_Layout + " ELECT_STMTIN路徑為 " + FILE_PATH;
                    logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                }
                ...
                if (Check_RC.Substring(0, 5) != "S0000")
                {
                    if (ELECT_PAY_DTE.Contains(TODAY_PROCESS_DTE.ToString("yyyyMMdd")))
                    {
                        #region 發提示Email
                        //Email主旨
                        string email_subject = "::: CyberCArd Message for PUBLIC_IN/PBBELI001  FILE NOTICE  :::  (Notice ONLY!)";
                        //Email內容
                        #region Email內容
                        string email_body = "此Mail僅為通知，批次仍在執行中！           " + "</br>" + "</br>" + "</br>" +
                                            "TIME     = " + DateTime.Now.ToString() + "</br>" +
                                            "今日為台電約定扣款日, 但無台電扣款檔案，請注意前一工作日是否已處理，謝謝。";
                        #endregion
                ...
                using (StreamReader srInFile = new StreamReader(strOutFileName, BIG5))
                {
                    ...
                    while ((strInfData = srInFile.ReadLine()) != null && PROCESS_FLAG == "Y")
                    {
                        ...
                        {
                            logger.strJobQueue = "台電(一般電)檔中第" + intReadInfDataCount + "筆的長度有誤，請通知系統人員! 該筆實際長度為 " + strInfData.Length;
                            logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                            return "B0099:" + logger.strJobQueue;
                        }
                        ...
                        switch (strTag)
                        {
                            case "1":  //檔頭資料
                                ...
                                if (PROCESS_FLAG == "Y")
                                {
                                    ...
                                    #region 刪除今日新增之台電(一般電)代繳清單(rerun準備)
                                    PUBLIC_LIST.init();
                            ...
                            case "2":  //明細資料
                                ...
                                PAY_NBR = Convert.ToString(InfData_DataTable.Rows[0]["ELECT_NBR"]);
                                ...
                                if (Convert.ToString(InfData_DataTable.Rows[0]["COND_CODE"]) != "99")   //台電通知止扣
                                {
                                ...
                                PUBLIC_HIST_T.resultTable.Rows[x]["MNT_USER"] = "PBBELI001";
                            ...
                            default: //資料庫錯誤
                                return "B0016:" + "台電(一般電)代扣繳 REC_TYPE ERROR " + strTag;
        ...
        void writeDisplay()
        {
            ...
            //今日處理之台電(一般電)資料
            logger.strTBL_NAME = "ELECT_STMTIN";
