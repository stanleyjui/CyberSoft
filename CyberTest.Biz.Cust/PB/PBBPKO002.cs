...
{
    /// <summary>
    /// 產生停車費退費交易(台北市、台北縣、台南市、高雄市)扣帳回應檔,並將入帳交易先寫至TX_WAREHOUSE,待下一營業日入帳
    /// </summary>
    public class PBBPKO002
    {
        ...
        #region 宣告常數
        const string strJOBNAME = "PBBPKO002";
        #endregion
        ...
        public string RUN(String strSOURCE)
        {
            ...
            try
            {
                ...
                TEMP_PROCESS_YYYMMDD = Convert.ToString(Convert.ToInt32(PROCESS_DATE.ToString("yyyy")) - 1911) + PROCESS_DATE.ToString("MMdd");
                #endregion
                ...
                switch (strSOURCE.ToString().Trim())
                {
                    case "TPE":     //台北市停車費
                        //收取檔案路徑
                        FILE_PATH = CMCURL.getPATH("TPE_RTROUT");
                        PAY_TYPE = "0023";
                        SOURCE_CODE = "PU00000023";
                        CAR_DESC = "台北市停車費退費";
                        REPORT_NAME = "PBRTPE002(台北市停車費退費成功失敗報表)";
                        break;

                    case "TPC":     //台北縣停車費
                        //收取檔案路徑
                        FILE_PATH = CMCURL.getPATH("TPC_RTROUT");
                        PAY_TYPE = "0021";
                        SOURCE_CODE = "PU00000021";
                        CAR_DESC = "台北縣停車費退費";
                        REPORT_NAME = "PBRTPC002(台北縣停車費退費成功失敗報表)";
                        break;

                    case "PRK":     //台南市停車費
                        //收取檔案路徑
                        FILE_PATH = CMCURL.getPATH("PRK_RTROUT");
                        PAY_TYPE = "0006";
                        SOURCE_CODE = "PU00000006";
                        CAR_DESC = "台南市停車費退費";
                        REPORT_NAME = "PBRPRK002(台南市停車費退費成功失敗報表)";
                        break;

                    case "KAO":     //高雄市停車費
                        //收取檔案路徑
                        FILE_PATH = CMCURL.getPATH("KAO_RTROUT");
                        PAY_TYPE = "0022";
                        SOURCE_CODE = "PU00000022";
                        CAR_DESC = "高雄市停車費退費";
                        REPORT_NAME = "PBRKAO002(高雄市停車費退費成功失敗報表)";
                        break;
        ...
        void MAIN_ROUTINE(String strSOURCE)
        {
            ...
            PUBLIC_LIST.init();
            PUBLIC_LIST_RC = PUBLIC_LIST.query_for_out_RTN(PAY_TYPE, TODAY_PROCESS_DTE);
            ...
            //REC Layout
            DataTable REC_DataTable = xml.Xml2DataTable(strXML_Layout, "REC_R");
            ...
            //REC_T Layout
            DataTable REC_T_DataTable = xml.Xml2DataTable(strXML_Layout, "REC_RT");
            ...
            for (i = 0; i < PUBLIC_LIST_Query_Count; i++)
            {
                ...
                switch (strTag)
                {
                    case "1":  //檔頭資料       
                        ...
                        TX_WAREHOUSE.strWhereMNT_USER = "PBBPKO002";
                    ...
                    case "2":  //明細資料
                        ...
                        if (PAY_RESULT == "S000")
                        {
                            ...
                            CAR_STMTOUT[i] = PAY_DATA_AREA.Substring(0, 60) + BANK_NEXT_PROCESS_DTE.ToString("yyyyMMdd").Substring(3,4) + PAY_RESULT_CAR + new string(' ', 64);
                            ...
                            if (TX_AMT > 0)
                            {
                                ...
                                TX_WAREHOUSE_T.resultTable.Rows[x]["CODE"] = "0002";
                                ...
                                TX_WAREHOUSE_T.resultTable.Rows[x]["DESCR"] = CAR_DESC;
                                TX_WAREHOUSE_T.resultTable.Rows[x]["MT_TYPE"] = "C";
                                TX_WAREHOUSE_T.resultTable.Rows[x]["AMT"] = TX_AMT;
                                TX_WAREHOUSE_T.resultTable.Rows[x]["MNT_DT"] = TODAY_PROCESS_DTE;
                                TX_WAREHOUSE_T.resultTable.Rows[x]["MNT_USER"] = "PBBPKO002";
                            }
                            #endregion
                            ...
                            PUBLIC_HIST.strMNT_USER = "PBBPKO002";
                    ...
                    case "3": //檔尾資料
                        ...
                        //TEMP_FAIL_AMT = Convert.ToInt32(CAR_FAIL_AMT).ToString().PadLeft(8, '0');
                        string strTOTAL_CNT = string.Empty.PadLeft(13, '0');
                        string strTOTAL_AMT = string.Empty.PadLeft(8, '0');
                        string strTOTAL_AMT_REJECT = string.Empty.PadLeft(13, '0');
                        string strTOTAL_CNT_REJECT = string.Empty.PadLeft(8, '0');

                        CAR_STMTOUT[i] = '3' + strTOTAL_CNT + strTOTAL_AMT
                                       + strTOTAL_AMT_REJECT + strTOTAL_CNT_REJECT + new string(' ', 87);
                        #endregion

                        break;

                    default: //資料庫錯誤
                        throw new System.Exception("B0016：" + "停車費退費 REC_TYPE ERROR " + strTag + logger.strJobQueue);
        ...
        void insertReport_TABLE()
        {
            ...
            {
                case "S000":
                    REPORT_TABLE.Rows[k]["PAY_RESULT"] = PAY_RESULT + " - 退費成功";
                    break;
        ...
        void writeReport()
        {
            ...
            alSumData.Add(new ExcelFactory.ListItem("#REPORT_NAME", CAR_DESC + "成功/失敗報表"));
