...
namespace Cybersoft.Biz.Cust
{
    ...
    public class PBBTWI001
    {
        ...
        DateTime dtConvert_DTE = new DateTime();

        //序號組合
        String TRANS_DTE = "";
        String PAY_TYPE = "0003";   //省水
        ...
        //定義每月付款日
        String PAY_DATE = "yyyymm07,yyyymm12,yyyymm17";
        String PAY_FLAG = "";
        ...
        public string RUN(string BATCH_NO)
        {
            ...
            try
            {
                ...
                #region 判斷今天是否為扣款日
                PAY_DATE = PAY_DATE.Replace("yyyymm", TODAY_PROCESS_DTE.ToString("yyyyMM"));
                char[] chr = { ',' };
                string[] strPAY_DATE = PAY_DATE.Split(chr);
                for (int a = 0; a < strPAY_DATE.Length; a++)
                {
                    logger.strJobQueue = "PAY_DATE : " + strPAY_DATE[a];
                    logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                    DateTime.TryParseExact(strPAY_DATE[a], "yyyyMMdd", new CultureInfo("zh-TW", true), DateTimeStyles.None, out dtConvert_DTE);
                    if ((TODAY_PROCESS_DTE <= dtConvert_DTE) && (dtConvert_DTE < NEXT_WORKING_DTE))
                    {
                        PAY_FLAG = "Y";
                        logger.strJobQueue = "今日為扣款日";
                        logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                    }
                    else
                    {
                        logger.strJobQueue = "今日非扣款日";
                        logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                    }
                }
                #endregion
                ...
                //收取檔案路徑
                FILE_PATH = CMCURL.getPATH("TWWATER_STMTIN");

                if (strXML_Layout == "" || FILE_PATH == "")
                {
                    logger.strJobQueue = "路徑取得錯誤!!! PUBLIC_TWWATER.xml路徑為 " + strXML_Layout + " TWWATER_STMTIN路徑為 " + FILE_PATH;
                    logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                ...
                else
                {
                    ...
                    logger.strJobQueue = "PUBLIC_TWWATER.xml路徑為 " + strXML_Layout + " TWWATER_STMTIN路徑為 " + FILE_PATH;
                    logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                }
                ...
                if (Check_RC.Substring(0, 5) != "S0000")
                {
                    ...
                    {
                        //Email主旨
                        string email_subject = ">> CyberCArd[info] PUBLIC_IN/PBBTWI001 FILE NOTICE (Notice ONLY!)";
                        //Email內容
                        #region Email內容
                        string email_body = "此Mail僅為通知，批次仍在執行中！           " + "</br>" + "</br>" + "</br>" +
                                            "TIME     = " + DateTime.Now.ToString() + "</br>" +
                                            "今日為省水約定扣款日, 但無省水扣款檔案，請注意前一工作日是否已處理，謝謝。";
                        #endregion
                ...
                using (StreamReader srInFile = new StreamReader(strOutFileName, BIG5))
                {
                    ...
                    while ((strInfData = srInFile.ReadLine()) != null && PROCESS_FLAG == "Y")
                    {
                        ...
                        {
                            logger.strJobQueue = "省水檔中第" + intReadInfDataCount + "筆的長度有誤，請通知系統人員! 該筆實際長度為 " + strInfData.Length;
                            logger.writeSysout(Cybersoft.Coca.Log.LevelEnum.Debug);
                            return "B0099:" + logger.strJobQueue;
                        }
                        ...
                        switch (strTag)
                        {
                            ...
                            case "2":  //明細資料
                                ...
                                if (Convert.ToString(InfData_DataTable.Rows[0]["REASON_CODE"]) != "99")   //公用事業單位通知止扣
                                {
                                    PUBLIC_HIST_T.resultTable.Rows[x]["PAY_RESULT"] = "I008";
                                    PUBLIC_HIST_T.resultTable.Rows[x]["USER_FIELD_2"] = Convert.ToString(InfData_DataTable.Rows[0]["REASON_CODE"]);
                                }
                                PUBLIC_HIST_T.resultTable.Rows[x]["MNT_DT"] = TODAY_PROCESS_DTE;
                                PUBLIC_HIST_T.resultTable.Rows[x]["MNT_USER"] = "PBBTWI001";
        ...
        void writeDisplay()
        {
            ...
            //今日處理之省水資料
            logger.strTBL_NAME = "TWWATER_STMTIN";
